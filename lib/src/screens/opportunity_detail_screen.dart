...
class OpportunityDetailScreen extends ConsumerWidget {
  ...
  Future<void> _submitApplication(BuildContext context, WidgetRef ref) async {
    ...
    if (authState == null || userProfile == null) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please sign in to apply.')));
      }
      return;
    }
          ...
          children: [
            Container(
              ...
              child: Column(
                ...
                children: [
                  Text(opportunity.title, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Color(0xFF111827))),
                  const SizedBox(height: 10),
                  Text(opportunity.organization, style: const TextStyle(fontSize: 16, color: Color(0xFF475569))),
                  const SizedBox(height: 16),
                  Row(
                    children: [
                      _DetailBadge(icon: Icons.location_on, label: opportunity.location),
                      const SizedBox(width: 10),
                      _DetailBadge(icon: Icons.schedule, label: opportunity.timeCommitment),
                      const SizedBox(width: 10),
                      _DetailBadge(icon: Icons.calendar_today, label: '${daysAgo}d ago'),
                    ],
                  ),
                  const SizedBox(height: 16),
                  Wrap(
                    spacing: 10,
                    runSpacing: 10,
                    children: opportunity.tags.map((tag) => Chip(label: Text(tag), backgroundColor: const Color(0xFFF8FAFC), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)))).toList(),
                  ),
                  const SizedBox(height: 18),
                  const Text('Description', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Color(0xFF111827))),
                  const SizedBox(height: 10),
                  Text(opportunity.description, style: const TextStyle(fontSize: 15, height: 1.6, color: Color(0xFF475569))),
